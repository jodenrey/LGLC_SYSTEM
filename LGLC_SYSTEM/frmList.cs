...
namespace LGLC_SYSTEM
{
    public partial class frmList : Form
    {
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            string sql = "Select * from tbl_Student where LName like '" + txtSearch.Text + "%'";
            cm = new SqlCommand(sql, cn);
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
                list = studentList.Items.Add(dr.GetValue(0).ToString());
                list.SubItems.Add(dr.GetValue(1).ToString() + " " + dr.GetValue(2).ToString() + " " + dr.GetValue(3).ToString() + " " + dr.GetValue(5).ToString());
